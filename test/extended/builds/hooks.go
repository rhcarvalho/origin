// this is a copy of test/extended/builds/start.go with few initial changes to
// implement an extended test for build hooks
...
var _ = g.Describe("[builds][Slow] builds with hooks", func() {
	defer g.GinkgoRecover()
	var (
		oc      = exutil.NewCLI("cli-start-build", exutil.KubeConfigPath())
		fixture = exutil.FixturePath("testdata", "build-hooks")
	)

	// REVIEW: Why do we need this run multiple times?! When exactly is is run?
